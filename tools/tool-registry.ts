...
function discoverPythonTools(): ClawCoreToolEntry[] {
  try {
    const script = `
import sys
import json
...
`;
    ...
    return JSON.parse(result.trim()) as ClawCoreToolEntry[];
  } catch (err) {
...
export function createToolRegistryService(api: any): ToolRegistryService {
...
`;
              ...
              try {
                ...
                return {
                  content: [{ type: 'text' as const, text: `Execution failed: ${execErr.message}` }],
                  details: {},
                  isError: true,
                };
              }
