...
// =====================================================================
// Self-Doubt Recall - Shared Context Store (from ZCrystal index.ts)
// =====================================================================
...
function setRecallContext(ctx: RecallContext): void {
  ...
  setTimeout(() => {
    if (pendingRecallContext === ctx) pendingRecallContext = null;
...
let zcState: ZCrystalState | null = null;
...
class SkillManager {
  ...
  async discover(_skillsDir: string): Promise<SkillEntry[]> {
    ...
    for (const [slug, content] of Object.entries(SKILL_CONTENT)) {
      ...
      const entry: SkillEntry = {
        slug,
        name:
          SKILL_CONTENT[slug as keyof typeof SKILL_CONTENT]
            ?.split('\n')
  ...
  getSkill(slug: string): SkillEntry | undefined {
...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerService(toolRegistryService as any);
    api.registerService(insightsService as any);
    ...
    try {
      ...
      zcState = {
        router, honcho, skillManager: skillManagerZCrystal as any, selfEvolution, evolutionCoordinator,
    ...
    if (zcState) {
      registerCoreTools(api, zcState as any);
      registerTaskTools(api, zcState as any);
      registerSkillTools(api, zcState as any);
    ...
    if (zcState) {
      ...
      api.registerTool({
        ...
        async execute(_id, _args) { const args = _args as any;
          if (!zcState) return errResult('Plugin not initialized');
      ...
      api.registerTool({
        ...
        async execute(_id, _params) {
          ...
          if (schema) return okResult(JSON.stringify(schema, null, 2));
      ...
      api.registerTool({
        ...
        async execute(_id, _params) {
          const params = _params as any;
          if (!zcState) return errResult('Plugin not initialized');
          const generated = await zcState.skillGenerator.generateFromTask(
            params.taskType, params.toolChain, params.parameters || {},
            params.examples || [], params.edgeCases || []
          );
          if (generated) return okResult(JSON.stringify(generated, null, 2), { skillId: generated.skillId });
      ...
      api.registerTool({
        ...
        async execute(_id, _params) {
          const params = _params as any;
          if (!zcState) return errResult('Plugin not initialized');
          if (params.level === 'info') zcState.logger.info(params.message, params.context || {});
          else if (params.level === 'warn') zcState.logger.warning(params.message, params.context || {});
          else if (params.level === 'error') zcState.logger.error(params.message, params.context || {});
      ...
      api.registerTool({
        ...
        async execute(_id, _params) {
          const params = _params as any;
          if (!zcState || !zcState.evolutionScheduler) return errResult('Plugin not initialized or scheduler unavailable');
          if (params.action === 'start') {
            ...
            const running = (zcState.evolutionScheduler as unknown as { running?: boolean }).running;
    ...
    api.registerCommand({
      ...
      async handler() {
        if (!zcState) return { text: 'Plugin not initialized (ZCrystal degraded mode)' };
...
- FTS5: ${fts5.success ? '✅ Available' : '❌ Unavailable'}` };
    ...
    api.registerHook('message:received', async (event: unknown) => {
      ...
      if (msg) {
        ...
        const rawEvent = event as { sender_label?: string; channel?: string; session_key?: string; message_id?: string; timestamp?: string };
        try {
    ...
    api.registerHook('message:preprocessed', async (event: unknown) => {
      if (!zcState) return;
      const ctx = event as { messages?: Array<{ role: 'user' | 'assistant' | 'system'; content: string; timestamp?: number }>; [key: string]: unknown };
      const messages = ctx.messages;
      if (!messages || messages.length === 0) return;

      const beforeCount = messages.length;
      const beforeTokens = messages.reduce<number>((sum, m) => sum + estimateTokens(m.content), 0);
      const compressed = compressContext(messages);
      const afterTokens = compressed.reduce<number>((sum, m) => sum + estimateTokens(m.content), 0);
    ...
    api.on('after_tool_call', async (event: unknown) => {
      ...
      if (classification.severity === 'high') console.error(logLine);
    ...
    api.on('llm_output', async (event: unknown) => {
      ...
      for (const text of texts) {
        ...
        const words = context.split(/[\s,，。!?]+/).filter((w: string) => w.length > 2);
    ...
    api.on('before_prompt_build', async (event: unknown) => {
      ...
      try {
        ...
        let searchResults = '';
        py.stdout.on('data', (d: Buffer) => searchResults += d.toString());
        py.stderr.on('data', (d: Buffer) => console.warn('[ZCrystal:self-doubt]', d.toString()));
        await new Promise<void>((resolve, reject) => {
          py.on('close', (code: number | null) => { if (code === 0) resolve(); else reject(new Error('exit ' + code)); });
          py.on('error', reject);
        });
