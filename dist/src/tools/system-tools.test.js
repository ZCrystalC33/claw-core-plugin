...
describe('System Tools', () => {
    ...
    it('should register zcrystal_review_stats', async () => {
        ...
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_review_stats')).toBe(true);
    });
    it('should register zcrystal_review_suggestions', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_review_suggestions')).toBe(true);
    });
    it('should register zcrystal_review_record', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_review_record')).toBe(true);
    });
    // Hook Tools
    it('should register zcrystal_hook_register', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_hook_register')).toBe(true);
    });
    it('should register zcrystal_hook_dispatch', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_hook_dispatch')).toBe(true);
    });
    it('should register zcrystal_hook_list', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_hook_list')).toBe(true);
    });
    // Evolution Coordinator Tools
    it('should register zcrystal_coordinator_status', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_coordinator_status')).toBe(true);
    });
    it('should register zcrystal_coordinator_register', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_coordinator_register')).toBe(true);
    });
    it('should register zcrystal_coordinator_evolve', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_coordinator_evolve')).toBe(true);
    });
    it('should register zcrystal_coordinator_queue', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_coordinator_queue')).toBe(true);
    });
    it('should register zcrystal_scheduler_start', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_scheduler_start')).toBe(true);
    });
    it('should register zcrystal_scheduler_stop', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        expect(mockApi.getTools().some((t) => t.name === 'zcrystal_scheduler_stop')).toBe(true);
    });
    ...
    it('should execute zcrystal_review_stats', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        const tool = mockApi.getTools().find((t) => t.name === 'zcrystal_review_stats');
    ...
    it('should execute zcrystal_hook_register successfully', async () => {
        const { registerSystemTools } = await import('./system-tools.js');
        registerSystemTools(mockApi, mockState);
        const tool = mockApi.getTools().find((t) => t.name === 'zcrystal_hook_register');
