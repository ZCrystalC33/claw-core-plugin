...
export function registerSignalTools(api: OpenClawPluginApi, _state: any) {
  ...
  api.registerTool({
    ...
    async execute(_id, _params) {
      const p = _params as any;
      try {
        const all = Array.from(store['signals'].values());
        const signals = p.activeOnly ? all.filter((s: any) => s.active !== false) : all;
  ...
  api.registerTool({
    ...
    async execute(_id, _params) {
      ...
      try {
        ...
        if (!signal) return errResult(`Signal not found: ${p.id}`);
  ...
  api.registerTool({
    ...
    async execute(_id, _params) {
      ...
      try {
        ...
        if ('error' in result) return errResult(String(result.error));
    ...
    async execute(_id, _params) {
      try {
        ...
        return okResult('Signal stats', {
          ...
          active: signals.filter((s: any) => s.active !== false).length,
        });
      } catch (err) {
        return errResult(String(err));
      }
