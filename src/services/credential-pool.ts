...
export interface CredentialPoolConfig {
  failureThreshold?: number;    // failures before circuit opens (default: 5)
  successThreshold?: number;    // successes to close circuit (default: 3)
  resetWindowMs?: number;       // window to reset failure count (default: 5 min)
  maxUsagePerKey?: number;      // max calls per key before rotation (default: 10000)
...
interface PoolState {
  ...
  config: Required<CredentialPoolConfig>;
  initialized: boolean;
...
function makeResult(text: string, details?: unknown) {
  return { content: [{ type: 'text' as const, text }], details: details ?? {} };
}

function makeErr(text: string) {
  return { content: [{ type: 'text' as const, text }], details: {}, isError: true };
}

function getDefaults(): Required<CredentialPoolConfig> {
  return {
    failureThreshold: 5,
    successThreshold: 3,
...
function getActiveKeyByProvider(state: PoolState, provider: string): CredentialKey | undefined {
  const candidates = [...state.keys.values()].filter(
    k => k.provider === provider && k.isActive && !k.isRotating
  );
...
function isCircuitOpen(key: CredentialKey, config: Required<CredentialPoolConfig>): boolean {
  if (key.consecutiveFailures < config.failureThreshold) return false;
  const now = Date.now();
  const windowStart = now - config.resetWindowMs;
  return key.lastFailureAt !== undefined && key.lastFailureAt > windowStart;
}

function checkCircuitHealth(key: CredentialKey, config: Required<CredentialPoolConfig>): void {
...
function recordUsage(state: PoolState, keyId: string, record: UsageRecord): void {
...
export function createCredentialPoolService(api: OpenClawPluginApi): OpenClawPluginService {
  return {
...
function registerCredentialTools(api: OpenClawPluginApi, state: PoolState): void {
...
export function getCredentialPoolState(): PoolState | null {
  return poolState;
}

export function getCredentialKey(provider: string): CredentialKey | undefined {
