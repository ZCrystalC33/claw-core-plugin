/**
 * observatory-tools.ts - Agent-facing observability tools
 * 
 * Provides tools for agents to:
 *   - Query observatory stats
...
function runPython(script: string, timeoutMs = 10000): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    py.on('close', (code) => { clearTimeout(timer); if (code === 0) resolve(stdout.trim()); else reject(new Error(stderr || `exit ${code}`)); });
    py.on('error', (e) => { clearTimeout(timer); reject(e); });
  });
}

function runPythonWithStdin(script: string, stdinData: unknown, timeoutMs = 10000): Promise<string> {
  return new Promise((resolve, reject) => {
    const py = spawn('python3', ['-c', script]);
    let stdout = '';
    let stderr = '';
...
async function recordOutcomeWE(context: string, strategy: string, outcome: string, tags: string[], score?: number): Promise<string> {
  const script = `
import sys
import json
...
async function getWeightedSuggestionsWE(tags: string[]): Promise<Array<[string, number]>> {
  const script = `
import sys
import json
sys.path.insert(0, '${WEIGHT_ENGINE_DIR}')
...
async function getStatsWE(): Promise<Record<string, { success_rate: number; total: number; avg_score: number }>> {
  const script = `
import sys
import json
sys.path.insert(0, '${WEIGHT_ENGINE_DIR}')
...
async function getAllEntriesWE(): Promise<Array<{ id: string; context: string; strategy: string; outcome: string; tags: string[]; timestamp: string; score: number }>> {
...
// ─── Observatory Stats (from log file) ────────────────────────────────────────
...
async function getObservatoryStats() {
  ...
  const entries: LogEntry[] = lines.map(l => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
  ...
  const catMap: Record<string, { total: number; success: number; duration: number }> = {};
  for (const e of entries) {
    if (!catMap[e.category]) catMap[e.category] = { total: 0, success: 0, duration: 0 };
    catMap[e.category].total++;
    if (e.outcome === 'success') catMap[e.category].success++;
  ...
  if (fs.existsSync(PATTERNS_DB)) {
    try {
      ...
      stats.topPatterns = Object.values(db.patterns || {}).sort((a: any, b: any) => b.totalAttempts - a.totalAttempts).slice(0, 5).map((p: any) => ({ pattern: p.pattern, successRate: p.successRate, totalAttempts: p.totalAttempts }));
    } catch { /* ignore */ }
  }
...
async function getRecentNotifications(limit = 10): Promise<any[]> {
...
async function executeParallelInternal(subtasks: Array<{ id: string; task: string; priority?: string }>): Promise<{ completed: string[]; failed: string[]; duration: number }> {
  const start = Date.now();
  const completed: string[] = [];
  const failed: string[] = [];

  // Simulate parallel execution with Promise.allSettled
  const results = await Promise.allSettled(
    subtasks.map(s => 
      new Promise<void>((resolve, reject) => {
        // Simulate task execution with random delay based on priority
        const delay = s.priority === 'high' ? 100 : s.priority === 'low' ? 500 : 250;
        setTimeout(() => {
          if (Math.random() > 0.1) resolve();
          else reject(new Error(`Simulated failure for ${s.id}`));
        }, delay);
      })
    )
  );
  ...
  subtasks.forEach((s, i) => {
    if (results[i].status === 'fulfilled') completed.push(s.id);
...
export function registerObservatoryTools(api: OpenClawPluginApi) {
  ...
  api.registerTool({
    ...
    async execute(_id, _params) {
      try {
        ...
        for (const [cat, data] of Object.entries(stats.byCategory as Record<string, { total: number; successRate: number; avgDuration: number }>)) {
  ...
  api.registerTool({
    ...
    async execute(_id, _params) { const params = _params as any;
      ...
      try {
        ...
        // Log the parallel execution as an outcome
        const fsSync = fs;
        const outcome = {
