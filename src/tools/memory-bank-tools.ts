...
export function registerMemoryBankTools(api: any) {
  // ─── MQI Search ───────────────────────────────────────────────
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      ...
      try {
        ...
        for (const r of results) {
          ...
          if (r.records?.length) {
            r.records.slice(0, 3).forEach((rec: Record<string, unknown>) => {
              const content = String(rec.content || '').slice(0, 150);
              lines.push(`   • ${content.replace(/\n/g, ' ')}`);
            });
            if (r.records.length > 3) lines.push(`   ... +${r.records.length - 3} more`);
          } else if (r.error) {
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      ...
      try {
        ...
        return okResult(
          `Compressed: ${params.messages.length} → ${result.compressed.length} messages, ` +
          `saved ~${result.savedTokens} tokens`,
          { before: params.messages.length, after: result.compressed.length, savedTokens: result.savedTokens }
        );
      } catch (e) {
